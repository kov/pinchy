...
syscall_handler!(read, read, args, data, {
    data.fd = args[0] as i32;
    data.count = args[2];

    let buf_addr = args[1];
    let to_read = core::cmp::min(data.count as usize, data.buf.len());
    unsafe {
...
syscall_handler!(write, args, data, {
    data.fd = args[0] as i32;
    data.count = args[2];

    let buf_addr = args[1];
    let to_copy = core::cmp::min(data.count as usize, data.buf.len());
    unsafe {
        let _ = bpf_probe_read_buf(buf_addr as *const _, &mut data.buf[..to_copy]);
    }
});

syscall_handler!(pread64, pread, args, data, {
    data.fd = args[0] as i32;
    data.count = args[2];
...
syscall_handler!(pwrite64, pwrite, args, data, {
    data.fd = args[0] as i32;
    data.count = args[2];
    data.offset = args[3] as i64;
