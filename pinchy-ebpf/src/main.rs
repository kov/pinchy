...
use pinchy_common::{
    ...
    syscalls::{
        ...
        SYS_recvmsg, SYS_rseq, SYS_rt_sigaction, SYS_rt_sigprocmask, SYS_sched_yield,
                ...
                if let Ok(msg_iovlen) = bpf_probe_read_user::<u32>(msg_ptr.add(24) as *const u32) {
                    ...
                    if msg_iov_addr != 0 && msghdr.msg_iovlen > 0 {
                        ...
                        for i in 0..iov_count {
                            ...
                            let iov_base = bpf_probe_read_user::<u64>(iov_ptr as *const u64).unwrap_or(0);
                            let iov_len = bpf_probe_read_user::<u64>(
                                (iov_ptr as *const u64).add(1)
                            ).unwrap_or(0);

                            msghdr.msg_iov[i] = pinchy_common::kernel_types::Iovec {
                                iov_base,
                                iov_len,
                            };
                        }
