    ...
    unsafe {
        ...
        *value_ptr = 42;
        ...
        let new_value = libc::itimerspec {
            it_interval: libc::timespec { tv_sec: 1, tv_nsec: 0 }, // 1 second interval
            it_value: libc::timespec { tv_sec: 2, tv_nsec: 500_000_000 }, // 2.5 second initial
        };
        ...
        if result < 0 {
            bail!("timer_getoverrun failed: {}", std::io::Error::last_os_error());
        }
        ...
        if result != 0 {
            bail!("timer_create with NULL sigevent failed: {}", std::io::Error::last_os_error());
        }
        ...
        if result != 0 {
            bail!("timer_delete for second timer failed: {}", std::io::Error::last_os_error());
        }
    }
