    ...
    match event.syscall_nr {
            ...
            match data.operation {
                crate::format_helpers::seccomp_constants::SECCOMP_GET_ACTION_AVAIL => {
                    if data.args == 0 {
                        ...
                        // Show the parsed action value (0 is a valid action: SECCOMP_RET_KILL_THREAD)
                        argf!(
                            sf,
                            "action: {}",
                ...
                crate::format_helpers::seccomp_constants::SECCOMP_SET_MODE_FILTER => {
                    if data.args == 0 {
                        ...
                        argf!(sf, "prog: {{len: {}, filter: 0x{:x}}}", data.filter_len, data.args);
                    } else {
                ...
                crate::format_helpers::seccomp_constants::SECCOMP_GET_NOTIF_SIZES => {
                    if data.args == 0 {
                        arg!(sf, "sizes: NULL");
                    } else if data.notif_sizes[0] != 0 || data.notif_sizes[1] != 0 || data.notif_sizes[2] != 0 {
        ...
        syscalls::SYS_kexec_load => {
            ...
            if data.segments_read > 0 {
                let segments: Vec<String> = data.parsed_segments
                    [..data.segments_read as usize]
                    .iter()
                    .map(|seg| {
                        format!(
                ...
                if data.segments_read < data.nr_segments
                    && data.nr_segments > pinchy_common::kernel_types::KEXEC_SEGMENT_ARRAY_CAP as u64
                {
                    argf!(
                        sf,
                        "segments: [{}... (showing {} of {})]",
