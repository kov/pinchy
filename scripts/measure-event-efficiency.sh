...
RUNS="${RUNS:-15}"
LOOPS="${LOOPS:-250}"
EVENTS="${EVENTS:-read,write,openat,close,lseek}"
WORKLOAD="${WORKLOAD:-benchmark_trace_loop}"
CLIENT_QUEUE_CAPACITY="${CLIENT_QUEUE_CAPACITY:-128}"
...
tmp_dir="$(mktemp -d /tmp/pinchy-eff-XXXXXX)"
keep_tmp="${KEEP_TMP:-1}"
...
echo "Running benchmark inside UML..."
"$uml_kernel" \
    mem=64M \
    root=/dev/root \
    rootfstype=hostfs \
    ...
    "PINCHY_TEST_WORKLOAD=$WORKLOAD" \
    "PINCHY_TEST_OUTDIR=$tmp_dir" \
    "PINCHY_TEST_BINDIR=$bindir" \
    "PINCHY_TEST_PROJDIR=$ROOT_DIR" \
    "PINCHY_TEST_MODE=benchmark" \
    "PINCHY_TEST_NAME=efficiency_benchmark" \
    "PINCHY_BENCH_LOOPS=$LOOPS" \
...
echo
echo "=== Pinchy Efficiency Measurement ==="
echo "events filter: $EVENTS"
echo "workload: $WORKLOAD"
echo "throughput loops: $LOOPS"
